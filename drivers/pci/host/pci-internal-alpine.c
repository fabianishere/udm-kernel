...
static int al_pcie_probe(struct platform_device *pdev)
{
	...
	err = of_pci_get_host_bridge_resources(np, 0, 0xff, &pci_resources, NULL);
	if (err)
		return err;

	msix_node = of_parse_phandle(np, "msi-parent", 0);
	if (msix_node) {
		al_pcie->msi = of_pci_find_msi_chip_by_node(msix_node);
		if (!al_pcie->msi)
			dev_err(al_pcie->dev,"annapurna labs msi chip not found!\n");
	} else {
		dev_err(al_pcie->dev,"annapurna labs msi-parent not found!\n");
	}

	dev_dbg(al_pcie->dev,"annapurna labs msi-parent: %p\n", al_pcie->msi);
