...
#ifdef AL_SSM_DMA_PARAMS_HAS_UNIT_ADAPTER
...
static int max_channels = AL_DMA_MAX_CHANNELS_V1_V2;
module_param(max_channels, int, 0644);
MODULE_PARM_DESC(
	max_channels,
	"maximum number of channels (queues) to enable (default: 4)");

static int max_channels_v3 = AL_DMA_MAX_CHANNELS_V3;
module_param(max_channels_v3, int, 0644);
MODULE_PARM_DESC(
	max_channels_v3,
	"maximum number of channels (queues) to enable (default: 16)");
...
#define AL_DMA_HW_QUEUES_BY_REV(rev_id) \
	(((rev_id) >= AL_RAID_REV_ID_3) ? max_channels_v3 : max_channels)
...
static int al_dma_read_pcie_config(void *handle, int where, uint32_t *val)
{
	...
	return pci_read_config_dword((struct pci_dev *)handle, where, val);
}

static int al_dma_write_pcie_config(void *handle, int where, uint32_t val)
{
	/* handle is a pointer to the pci_dev */
	return pci_write_config_dword((struct pci_dev *)handle, where, val);
}
#endif
	...
	void __iomem		*iobase_app)
{
	...
	device->max_channels = AL_DMA_HW_QUEUES_BY_REV(device->rev_id);
...
#ifdef AL_SSM_DMA_PARAMS_HAS_UNIT_ADAPTER
	device->ssm_dma_params.func_num = PCI_FUNC(device->pdev->devfn);
	device->ssm_dma_params.skip_adapter_init = AL_FALSE;
	device->ssm_dma_params.unit_adapter = devm_kzalloc(&device->pdev->dev,
							   sizeof(struct al_unit_adapter),
							   GFP_KERNEL);
	if (!device->ssm_dma_params.unit_adapter) {
		dev_err(device->common.dev, "devm_kzalloc failed\n");
		return -ENOMEM;
	}

	err = al_unit_adapter_handle_init(device->ssm_dma_params.unit_adapter,
					  AL_UNIT_ADAPTER_TYPE_SSM,
					  NULL,
					  al_dma_read_pcie_config,
					  al_dma_write_pcie_config,
					  NULL,
					  device->pdev);
	if (err) {
		dev_err(device->common.dev, "al_unit_adapter_handle_init failed\n");
		return err;
	}
#endif
	...
	device->ssm_dma_params.num_of_queues = device->max_channels;
	...
	al_dma_init_channels(device, device->max_channels);
...
#define FAST_DMA_DESCS_COUNT		64
#define FAST_DMA_TX_CDESCS_COUNT	64
#define FAST_DMA_RX_CDESCS_COUNT	64
...
al_phys_addr_t tx_dma_desc_phys[AL_DMA_MAX_CHANNELS_MAX];
al_phys_addr_t rx_dma_desc_phys[AL_DMA_MAX_CHANNELS_MAX];
al_phys_addr_t rx_dma_cdesc_phys[AL_DMA_MAX_CHANNELS_MAX];
void *tx_dma_desc_virt[AL_DMA_MAX_CHANNELS_MAX];
void *rx_dma_desc_virt[AL_DMA_MAX_CHANNELS_MAX];
void *rx_dma_cdesc_virt[AL_DMA_MAX_CHANNELS_MAX];
...
static int al_dma_setup_interrupts(struct al_dma_device *device)
{
	...
	ret = pci_enable_msix_range(pdev, device->msix_entries, msixcnt, msixcnt);
	if (ret != msixcnt)
...
#if defined(AL_UDMA_IOFIC_HAS_CONFIG_EX)
	err = al_udma_iofic_config_ex(&device->hal_raid.m2m_udma.tx_udma, &cfg);
#else
	...
	al_udma_iofic_unmask_adv(
		&device->hal_raid.m2m_udma.tx_udma,
...
msix_single_vector:
	...
	msixcnt = 1; /* if we can't allocate all, then try only 1; */
	ret = pci_enable_msix_range(pdev, device->msix_entries, msixcnt, msixcnt);
	if (ret != msixcnt)
		goto intx;
...
#if defined(AL_UDMA_IOFIC_HAS_UNMASK_ADV)
	al_udma_iofic_unmask_adv(
		&chan->device->hal_raid.m2m_udma.tx_udma,
		AL_UDMA_IOFIC_LEVEL_PRIMARY,
		AL_INT_GROUP_B,
		1 << chan->idx);
#else
