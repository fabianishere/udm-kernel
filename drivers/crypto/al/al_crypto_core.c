...
/******************************************************************************
 *****************************************************************************/
static void al_crypto_ext_tgtid_setup(struct al_crypto_device *device, unsigned int tgtid)
{
	/* in pcie external mode, force eth UDMA to access PCIE0 using the vmid */
	struct al_udma_gen_tgtid_conf	conf;
	struct al_udma_gen_tgtid_msix_conf msix_conf;
	int i;
	struct udma_iofic_regs *int_regs;

	memset(&conf, 0, sizeof(struct al_udma_gen_tgtid_conf));
...
#if defined(AL_UDMA_GEN_TGTID_HAS_CONF_SET_ADV)
	al_udma_gen_tgtid_conf_set_adv(&device->hal_crypto.m2m_udma.tx_udma, &conf);
#else
	al_udma_gen_tgtid_conf_set(device->udma_regs_base, &conf);
#endif

	/* force MSIX to access PCIE 0 using the vmid */
	msix_conf.access_en = AL_TRUE;
	msix_conf.sel = AL_TRUE;
#if defined(AL_UDMA_GEN_TGTID_MSIX_HAS_CONF_SET_ADV)
	al_udma_gen_tgtid_msix_conf_set_adv(&device->hal_crypto.m2m_udma.tx_udma, &msix_conf);
#else
	al_udma_gen_tgtid_msix_conf_set(device->udma_regs_base, &msix_conf);
#endif

	int_regs = al_udma_gen_int_regs_get(&device->hal_crypto.m2m_udma.tx_udma);
	al_iofic_msix_tgtid_attributes_config(&int_regs->main_iofic,
		...
		void __iomem		*iobase_app)
{
	...
	if ((crc_channels > max_channels) || (max_channels > DMA_MAX_Q_MAX)) {
	...
	device->crypto_regs_base = iobase_app ? iobase_app +
			AL_CRYPTO_APP_REGS_BASE_OFFSET(device->rev_id)
			: NULL;
	...
	if (!al_crypto_device_is_onchip(device))
		al_crypto_ext_tgtid_setup(device, device->ext_info.tgtid);
...
#if !defined(AL_UDMA_IOFIC_HAS_UNMASK_ADV)
...
#if defined(AL_UDMA_IOFIC_HAS_UNMASK_ADV)
	al_udma_iofic_unmask_adv(&device->hal_crypto.m2m_udma.tx_udma, AL_UDMA_IOFIC_LEVEL_PRIMARY,
		...
		bool single_msix)
{
...
#if defined(AL_UDMA_IOFIC_HAS_CONFIG_EX)
	struct al_udma_iofic_cfg cfg = {
		.mode = AL_IOFIC_MODE_MSIX_PER_Q,
		.m2s_errors_disable = 0x480,
		.m2s_aborts_disable = 0x480,
		.s2m_errors_disable = 0x1E0,
		.s2m_aborts_disable = 0x1E0,
	};
#endif
...
#if defined(AL_UDMA_IOFIC_HAS_CONFIG_EX)
	cfg.mode = iofic_mode;
	err = al_udma_iofic_config_ex(&device->hal_crypto.m2m_udma.tx_udma, &cfg);
#else
	err = al_udma_iofic_config((struct unit_regs *)device->udma_regs_base, iofic_mode, 0x480,
				   0x480, 0x1E0, 0x1E0);
#endif
	if (err) {
	...
	int_regs = al_udma_gen_int_regs_get(&device->hal_crypto.m2m_udma.tx_udma);
	al_iofic_moder_res_config(&int_regs->main_iofic,
...
static int al_crypto_setup_interrupts(struct al_crypto_device *device)
{
	 ...
	 */
	if (!al_crypto_device_is_onchip(device))
		writew(PCI_MSIX_FLAGS_ENABLE, device->ext_info.internal_pcie_base +
			0x1000 * device->ext_info.udma_num + 0x92);
...
intx:
	...
	err = al_udma_iofic_config_ex(&device->hal_crypto.m2m_udma.tx_udma, &cfg);
#else
	err = al_udma_iofic_config((struct unit_regs *)device->udma_regs_base, AL_IOFIC_MODE_LEGACY, 0x480, 0x480, 0x1E0, 0x1E0));
#endif
	if (err) {
...
static int al_crypto_alloc_chan_resources(struct al_crypto_chan *chan)
{
	...
	if (chan->tx_dma_desc_virt == NULL) {
		dev_err(dev, "failed to allocate %u bytes of coherent memory for Tx submission descriptors\n",
			(unsigned int)
	...
	if (chan->rx_dma_desc_virt == NULL) {
		dev_err(dev, "failed to allocate %u bytes of coherent memory for Rx submission descriptors\n",
			(unsigned int)
...
#if defined(AL_UDMA_IOFIC_HAS_MASK_ADV)
	al_udma_iofic_mask_adv(
		&device->hal_crypto.m2m_udma.tx_udma,
...
#if defined(AL_UDMA_IOFIC_HAS_READ_CAUSE_ADV)
	read_cause_group_d = al_udma_iofic_read_cause_adv(&device->hal_crypto.m2m_udma.tx_udma,
...
#if defined(AL_UDMA_IOFIC_HAS_UNMASK_ADV)
	al_udma_iofic_unmask_adv(
		&chan->device->hal_crypto.m2m_udma.tx_udma,
		AL_UDMA_IOFIC_LEVEL_PRIMARY,
		AL_INT_GROUP_B,
		1 << chan->idx);
#else
...
#if defined(AL_UDMA_IOFIC_HAS_READ_CAUSE_ADV)
	status = al_udma_iofic_read_cause_adv(&device->hal_crypto.m2m_udma.tx_udma,
					      AL_UDMA_IOFIC_LEVEL_PRIMARY, group);
#else
...
void al_crypto_set_int_moderation(struct al_crypto_device *device, int interval)
{
	int i;
	struct udma_iofic_regs *int_regs =
		al_udma_gen_int_regs_get(&device->hal_crypto.m2m_udma.tx_udma);

	for (i = 0; i < device->num_channels; i++)
		al_iofic_msix_moder_interval_config(&int_regs->main_iofic, AL_INT_GROUP_B, i,
						    interval);
