...
#include "al_hal_eth_rfw_regs.h"
#include "al_hal_eth_rx_fwd_lrss_tables.h"
		...
		struct al_eth_rfw_handle_init_params *handle_init_params)
{
	...
	struct al_eth_rfw_cpu_regs __iomem *rfw_regs;

	al_assert(handle);
	al_assert(handle_init_params);
	al_assert(handle_init_params->cpu < AL_ETH_RFW_CPU_MAX);
	al_assert(handle_init_params->ec_regs_base);

	handle->cpu = handle_init_params->cpu;
	handle->ec_regs_base = handle_init_params->ec_regs_base;

	ec_regs = (struct al_ec_regs __iomem *)handle->ec_regs_base;

	rfw_regs = (struct al_eth_rfw_cpu_regs __iomem *)&ec_regs->rfw_shared_cfg[0];
	...
	struct al_eth_rfw_rss_table_params *params)
{
	...
	struct al_eth_rfw_cpu_regs *rfw_regs = handle->rfw_regs;

	addr_reg = &rfw_regs->lrss_indirection_table_addr;
	data_reg = &rfw_regs->lrss_indirection_table_data0;

	al_reg_write32(addr_reg, index);
	entry[0] = al_reg_read32(data_reg);
		...
		const struct al_eth_rfw_rss_table_params *params)
{
	...
	al_reg_write32(addr_reg, params->index);
	al_reg_write32(data_reg, entry[0]);

	return 0;
}

static const struct al_eth_rfw_lrss_tables_params default_lrss_tables_params = {
...
{
	const struct al_eth_rfw_lrss_tables_params *lrss_tables;
	uint64_t ind_table_total_size;
	unsigned int i;

	al_assert(lrss_handle);
	al_assert(handle_init_params);
	...
	if (lrss_tables->num > AL_ETH_RFW_LRSS_TABLES_MAX) {
		al_err("requested tables number is too large\n");
		return -EINVAL;
	}
	...
	if (lrss_tables->table_size_order > 16 || /* validate no overflow */
		...
		al_err("requested tables size is too large\n");
		return -EINVAL;
	}
...
uint32_t al_eth_rfw_lrss_ind_table_offset_get(
	const struct al_eth_rfw_lrss_handle *lrss_tables,
	uint32_t subtable_index)
{
	al_assert(lrss_tables);
	al_assert(subtable_index < lrss_tables->num);
...
{
	const struct al_eth_rfw_lrss_info *lrss_info;
	struct al_eth_rfw_rss_table_params rss_entry;
	int rc;

	al_assert(lrss_handle);
	al_assert(subtable_index < lrss_handle->num);
	al_assert(entry_index < (uint32_t)(1 << lrss_handle->table_size_order));

	lrss_info = &lrss_handle->lrss_info[subtable_index];
	/* check if entry_index is valid */
	al_assert((entry_index & lrss_info->hash_output_bit_mask) == 0);

	al_memset(&rss_entry, 0, sizeof(rss_entry));
	rss_entry.index = lrss_info->rss_ind_table_offset + entry_index;
	rss_entry.udma = AL_BIT(udma);
	rss_entry.queue = queue;
	rc = al_eth_rfw_lrss_table_update(lrss_handle->fwd_handle,
					    &rss_entry);
	return rc;
}

static void rfw_lrss_handle_init_helper(const struct al_eth_rfw_handle *rx_fwd_handle,
	struct al_eth_rfw_lrss_handle *lrss_tables_handle)
{
	int rc;
	struct al_eth_rfw_lrss_handle_init_params lrss_tables_handle_init_params = {
			.fwd_handle = rx_fwd_handle,
			.lrss_table_params = &default_lrss_tables_params,
	};

	al_assert(rx_fwd_handle);
	al_assert(lrss_tables_handle);

	al_memset(lrss_tables_handle, 0, sizeof(struct al_eth_rfw_lrss_handle));

	rc = al_eth_rfw_lrss_handle_init(lrss_tables_handle, &lrss_tables_handle_init_params);
	if (rc != 0) {
		al_err("%s: Failed at al_eth_rfw_lrss_handle_init (rc = %d)", __func__, rc);
		al_assert(0);
	}
}
	...
	struct al_eth_rfw_lrss_params *params)
{
	...
	al_assert(rx_fwd_handle);
	al_assert(params);
	al_assert(params->table_size <= (1 << AL_ETH_RFW_LRSS_SUBTABLE_ORDER));
	...
	for (i = 0; i < params->table_size; i++) {
		...
		if (rc != 0) {
			al_err("%s: Failed at al_eth_rfw_lrss_entry_set (rc = %d)", __func__, rc);
			return rc;
		}
	}
