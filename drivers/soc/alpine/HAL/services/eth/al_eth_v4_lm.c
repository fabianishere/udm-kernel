...
#define lm_debug(format, ...)					\
	do {							\
		if (handle->debug)				\
			al_warn(LM_PRI(format, ##__VA_ARGS__));	\
		else						\
			al_dbg(LM_PRI(format, ##__VA_ARGS__));	\
	} while (0)

#define lm_err(format, ...)					\
	al_err(LM_PRI(format, ##__VA_ARGS__));
 ...
 ******************************************************************************/
static int state_func_wait_mode_detection(struct al_eth_v4_lm *handle);
static int state_func_sfp_presence(struct al_eth_v4_lm *handle);
static int state_func_mac_pcs_config(struct al_eth_v4_lm *handle);
static int state_func_retimer_rx_config(struct al_eth_v4_lm *handle);
static int state_func_retimer_tx_config(struct al_eth_v4_lm *handle);
static int state_func_retimer_rx_cdr_reset(struct al_eth_v4_lm *handle);
static int state_func_retimer_tx_cdr_reset(struct al_eth_v4_lm *handle);
static int state_func_retimer_signal_detect(struct al_eth_v4_lm *handle);
static int state_func_retimer_cdr_lock(struct al_eth_v4_lm *handle);
static int state_func_serdes_rx_tx_reset(struct al_eth_v4_lm *handle);
static int state_func_serdes_rx_tx_reset_is_done(struct al_eth_v4_lm *handle);
static int state_func_retimer_rx_eq_done(struct al_eth_v4_lm *handle);
static int state_func_serdes_signal_detect(struct al_eth_v4_lm *handle);
static int state_func_serdes_ical_start(struct al_eth_v4_lm *handle);
static int state_func_serdes_ical_is_done(struct al_eth_v4_lm *handle);
static int state_func_serdes_cdr_lock(struct al_eth_v4_lm *handle);
static int state_func_serdes_pcal_adaptive_start(struct al_eth_v4_lm *handle);
static int state_func_link_check(struct al_eth_v4_lm *handle);
static int state_func_link_is_down(struct al_eth_v4_lm *handle);

/*******************************************************************************
 ** States
 ******************************************************************************/
enum al_eth_v4_lm_state {
 ...
 */
typedef int (*al_eth_v4_lm_state_func)(
	struct al_eth_v4_lm *);

/**
 * ETHv4 Link Management state parameters
 */
struct al_eth_v4_lm_state_params {
	/** State handling function */
	al_eth_v4_lm_state_func		func;
	 ...
	 */
	enum al_eth_v4_lm_state		timeout_auto_move_state;

	uint64_t			timeout_visit_req;
};

static const struct al_eth_v4_lm_state_params lm_states[AL_ETH_V4_LM_STATE_MAX] = {
...
static const char *state_to_str(enum al_eth_v4_lm_state state)
{
	switch (state) {
...
static void state_set(struct al_eth_v4_lm *handle, enum al_eth_v4_lm_state state)
{
	if ((enum al_eth_v4_lm_state)handle->state != state)
...
static uint64_t msec_to_time(struct al_eth_v4_lm *handle, uint64_t msec)
{
	...
	case AL_ETH_V4_LM_TIME_UNITS_USEC:
		return msec * 1000;
	default:
		al_assert_msg(AL_FALSE, "Unknown time unit\n");
		break;
	}
...
static al_bool state_is_timeout(struct al_eth_v4_lm *handle)
{
...
static unsigned int macs_num_get(struct al_eth_v4_lm *handle)
{
	enum al_eth_mac_mode mac_mode = handle->status.detected_mode.mac_mode;
	...
	case AL_ETH_MAC_MODE_CG_100G:
		break;
	default:
		al_assert_msg(AL_FALSE, "%s: Unsupported Ethernet MAC Mode: %s\n",
			__func__, al_eth_mac_mode_str(mac_mode));
		return 0;
	}
...
static unsigned int lanes_num_get(struct al_eth_v4_lm *handle)
{
	enum al_eth_mac_mode mac_mode = handle->status.detected_mode.mac_mode;
 ...
 *	ETHv4 Link Management handle
 */
static al_bool sfp_presence_check(struct al_eth_v4_lm *handle)
{
	...
	for (i = 0; i < lanes_num; i++) {
		struct al_eth_v4_lm_lane *lane = &handle->lanes[i];
		al_bool single_present;
...
static void led_set(struct al_eth_v4_lm *handle, al_bool link_up)
{
	unsigned int macs_num = macs_num_get(handle);
	unsigned int i;

	for (i = 0; i < macs_num; i++) {
		struct al_eth_mac_obj *mac_obj = handle->mac_objs[i];
		int err;
...
static int link_check(struct al_eth_v4_lm *handle)
{
	unsigned int macs_num = macs_num_get(handle);
	al_bool link_up_prev = handle->status.link_up;
	unsigned int i;
	int ret;
	...
	for (i = 0; i < macs_num; i++) {
		struct al_eth_mac_obj *mac_obj = handle->mac_objs[i];
		struct al_eth_link_status link_status;
	...
	if (handle->status.link_up) {
		if (handle->status.mode_detection_ongoing) {
			lm_debug("Detected mode is %s\n",
				al_eth_mac_mode_str(handle->status.detected_mode.mac_mode));
...
static int state_func_wait_mode_detection(struct al_eth_v4_lm *handle)
{
	static const enum al_eth_mac_mode std_mac_modes[] = {
		AL_ETH_MAC_MODE_10GbE_Serial
	};
	static const enum al_eth_mac_mode adv_mac_modes[] = {
...
static int state_func_sfp_presence(struct al_eth_v4_lm *handle)
{
	al_bool total_present_ready = AL_TRUE;
	al_bool total_present = AL_TRUE;
	al_bool bounce = AL_FALSE;
	unsigned int lanes_num = lanes_num_get(handle);
	unsigned int i;
	int ret = 0;
...
static int state_func_mac_pcs_config(struct al_eth_v4_lm *handle)
{
	enum al_eth_mac_mode mac_mode = handle->status.detected_mode.mac_mode;
	al_bool fec_enable;
	enum al_eth_fec_type fec_type = 0;
	unsigned int macs_num = macs_num_get(handle);
	unsigned int i;
	int ret;
...
static int state_func_retimer_rx_config(struct al_eth_v4_lm *handle)
{
	...
	for (i = 0; i < lanes_num; i++) {
		struct al_eth_v4_lm_lane *lane = &handle->lanes[i];
		struct al_eth_lm_retimer *retimer = &handle->retimers[lane->rx_retimer.idx];
...
static int state_func_retimer_tx_config(struct al_eth_v4_lm *handle)
{
	...
	for (i = 0; i < lanes_num; i++) {
		struct al_eth_v4_lm_lane *lane = &handle->lanes[i];
		struct al_eth_lm_retimer *retimer = &handle->retimers[lane->tx_retimer.idx];
		struct al_eth_lm_retimer_config_params config_params = {
	...
	const struct al_eth_v4_lm_params *params)
{
	...
	al_assert(handle);
	al_assert(params);
	al_assert(IS_ETH_ADV(params) || IS_ETH_STD(params));
	al_assert(params->mac_obj);
	al_assert(params->time_get);
	al_assert(params->step_max_time > 0);
	al_assert(params->lanes_num > 0 &&
		params->lanes_num <= AL_ETH_V4_LM_LANE_MAX);
	al_assert(params->retimers_num <= AL_ETH_V4_LM_RETIMER_MAX);
	...
	if (IS_ETH_ADV(params) && params->port_id == AL_ETH_V4_LM_ADV_MAIN_PORT) {
		al_assert(params->lanes_num == AL_ETH_V4_LM_LANE_MAX);

		for (i = 0; i < AL_ETH_V4_LM_MAC_MAX; i++) {
			al_assert(params->mac_objs[i]);

			handle->mac_objs[i] = params->mac_objs[i];
		}
	} else {
		al_assert(params->lanes_num == 1);
	}
	...
	if (params->retimers_num) {
		...
		for (i = 0; i < handle->retimers_num; i++) {
			const struct al_eth_lm_retimer_params *retimer_params =
				&params->retimers[i];
			struct al_eth_lm_retimer *retimer = &handle->retimers[i];

			al_eth_lm_retimer_handle_init(retimer, retimer_params);
		}
	} else {
		...
		al_eth_lm_retimer_handle_init(&handle->retimers[0], &retimer_params);
	}

	for (i = 0; i < handle->lanes_num; i++) {
		const struct al_eth_v4_lm_lane_params *lane_params = &params->lanes[i];
		struct al_eth_v4_lm_lane *lane = &handle->lanes[i];

		al_assert(lane_params->serdes_obj);
		al_assert(lane_params->rx_retimer.idx < handle->retimers_num);
		al_assert(lane_params->tx_retimer.idx < handle->retimers_num);
		al_assert(!lane_params->sfp_presence_exists || params->gpio_get);
	...
	al_assert(AL_ETH_V4_LM_STATE_MAX < 64);
...
{
	al_assert(handle);
	al_assert(IS_ETH_STD(handle) || handle->port_id == AL_ETH_V4_LM_ADV_MAIN_PORT);
...
static void al_eth_v4_timeout_state_req_set(struct al_eth_v4_lm *handle)
{
	al_dbg("States request after timeout for %s\n", state_to_str(handle->state));
	...
	struct al_eth_v4_lm *handle)
{
	...
	do {
		enum al_eth_v4_lm_state orig_state = handle->state;
		al_eth_v4_lm_state_func func = lm_states[orig_state].func;

		al_assert_msg(func, "Empty function for state '%s'\n", state_to_str(orig_state));
		...
		if (handle->state_sleep_request)
			al_udelay(handle->state_sleep_request);
