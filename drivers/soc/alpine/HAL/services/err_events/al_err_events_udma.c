 ...
 ** Static error fields
 ******************************************************************************/
static const struct al_err_events_field al_err_events_udma_fields[AL_ERR_EVENTS_UDMA_MAX_ERRORS] = {
	{
 ...
 ** UDMA
 ******************************************************************************/
static int err_events_udma_collect(struct al_err_events_module *module,
				   enum al_err_events_collect collect)
{
	...
	struct al_err_events_udma_data *data =
			(struct al_err_events_udma_data *)module->private_data;
	al_bool should_collect = AL_FALSE;

	cause =  al_iofic_read_cause(data->iofic_primary_regs_base, AL_INT_GROUP_D);
	...
	for (group = AL_INT_GROUP_A; group <= AL_INT_GROUP_C; group++) {
		...
		al_err_events_common_module_iofic_collect(module, collect, 0, group);
		cause_clr |= bit;
	}

	if (collect == AL_ERR_EVENTS_COLLECT_INTERRUPT)
		al_iofic_clear_cause(data->iofic_primary_regs_base, AL_INT_GROUP_D, cause_clr);

	return 0;
}

int al_err_events_udma_init(struct al_err_events_handle *handle,
			    struct al_err_events_udma_data *data,
			    struct al_err_events_udma_init_params *params)
{
	...
	al_assert(data);
	al_assert(params);
	al_assert(sizeof(al_err_events_udma_fields) == sizeof(data->fields));
	al_assert(sizeof(al_err_events_udma_fields) ==
		 (AL_ERR_EVENTS_UDMA_MAX_ERRORS * sizeof(struct al_err_events_field)));

	al_memset(&module_params, 0, sizeof(module_params));

	switch (params->primary_module) {
	case AL_ERR_EVENTS_MODULE_SSM:
		al_sprintf(module_params.name,
			"SSM(%d)-UDMA(%d)",
			params->module_index,
			params->udma_index);
		break;
	...
	/* init data */
	data->iofic_primary_regs_base = al_udma_iofic_reg_base_get_adv(&udma,
								   AL_UDMA_IOFIC_LEVEL_PRIMARY);
	data->ext_app_int = al_udma_iofic_get_ext_app_bit(&udma);
	data->sec_level_int[AL_INT_GROUP_A] =
		al_udma_iofic_sec_level_int_get(&udma, AL_INT_GROUP_A);
	data->sec_level_int[AL_INT_GROUP_B] =
		al_udma_iofic_sec_level_int_get(&udma, AL_INT_GROUP_B);
	if (al_udma_rev_id_get(&udma) >= AL_UDMA_REV_ID_4)
		data->sec_level_int[AL_INT_GROUP_C] =
			al_udma_iofic_sec_level_int_get(&udma, AL_INT_GROUP_C);
	else
	...
	module_params.ic_regs_bases[0] = al_udma_iofic_reg_base_get_adv(&udma,
	...
	module_params.fields_template = al_err_events_udma_fields;
	...
	if ((params->primary_module == AL_ERR_EVENTS_MODULE_SSM) ||
		(al_udma_rev_id_get(&udma) < AL_UDMA_REV_ID_4)) {

		unsigned int i;
		enum al_err_events_udma  ids[] = {
...
al_bool al_err_events_udma_ext_app_int_read_and_clear(struct al_err_events_udma_data *data)
{
	uint32_t cause;

	cause = al_iofic_read_and_clear_cause(data->iofic_primary_regs_base,
					      AL_INT_GROUP_D,
					      data->ext_app_int);
