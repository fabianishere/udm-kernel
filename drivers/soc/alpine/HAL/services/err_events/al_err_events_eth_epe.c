...
static const struct al_err_events_field
	al_err_events_eth_epe_fields[AL_ERR_EVENTS_ETH_EPE_MAX_ERRORS] = {
	{
 ...
 ** EC
 ******************************************************************************/
static int err_events_eth_epe_collect(struct al_err_events_module *module,
				     enum al_err_events_collect collect)
{
	int ret;
	unsigned int iofic_group;
	al_bool should_collect = AL_FALSE;
	struct al_err_events_eth_epe_data *epe_data = module->private_data;

	if (collect == AL_ERR_EVENTS_COLLECT_POLLING) {
		should_collect = AL_TRUE;
	} else if (collect == AL_ERR_EVENTS_COLLECT_INTERRUPT) {
		uint32_t cause = al_iofic_read_cause(epe_data->ec_intc_regs_base, AL_INT_GROUP_A);
	...
	if (collect == AL_ERR_EVENTS_COLLECT_INTERRUPT)
		al_iofic_clear_cause(epe_data->ec_intc_regs_base,
				AL_INT_GROUP_A,
...
int al_err_events_eth_epe_init(struct al_err_events_handle *handle,
			      struct al_err_events_eth_epe_data *data,
			      struct al_err_events_eth_epe_init_params *params)
{
	...
	al_assert(data);
	al_assert(params);
	al_assert(sizeof(al_err_events_eth_epe_fields) == sizeof(data->fields));
	al_assert(sizeof(al_err_events_eth_epe_fields) ==
		 (AL_ERR_EVENTS_ETH_EPE_MAX_ERRORS * sizeof(struct al_err_events_field)));
	al_assert(params->handle_params);
	al_assert(params->ec_intc_regs_base);
	...
	al_eth_epe_handle_init(&data->handle, params->handle_params);

	/* init module */
	al_sprintf(module_params.name, "ETH-EPE");
	...
	module_params.fields_template = al_err_events_eth_epe_fields;
	...
	al_eth_epe_error_ints_masks_get(&data->handle,
				       &a_mask,
				       &b_mask,
				       &c_mask,
				       &d_mask);
