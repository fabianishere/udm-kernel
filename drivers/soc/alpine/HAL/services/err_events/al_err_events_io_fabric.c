...
static void err_events_ua_axi_print(struct al_err_events_field *field, al_bool print_header);
static void err_events_ua_axi_parity_collect(struct al_err_events_field *field, uint32_t cause);
static void err_events_ua_axi_error_collect(struct al_err_events_field *field, uint32_t cause);
static void err_events_ua_axi_mask(struct al_err_events_field *field);

/*******************************************************************************
 ** Static error fields
 ******************************************************************************/
static const struct al_err_events_field al_err_events_pbs_fields[AL_ERR_EVENTS_PBS_MAX_ERRORS] = {
	{
...
static const struct al_err_events_field
	al_err_events_serdes_fields[AL_ERR_EVENTS_SERDES_MAX_ERRORS] = {
	{
...
static const struct al_err_events_field
	al_err_events_serdes_avg_fields[AL_ERR_EVENTS_SERDES_AVG_MAX_ERRORS] = {
	{
...
static const struct al_err_events_field
	al_err_events_ua_fields[AL_ERR_EVENTS_UNIT_ADAPTER_MAX_ERRORS] = {
	{
 ...
 ** PBS
 ******************************************************************************/
static int err_events_pbs_collect(struct al_err_events_module *module,
			   ...
			   struct al_err_events_pbs_init_params *params)
{
	...
	al_assert(data);
	al_assert(params);
	al_assert(sizeof(al_err_events_pbs_fields) == sizeof(data->fields));
	al_assert(sizeof(al_err_events_pbs_fields) ==
		 (AL_ERR_EVENTS_PBS_MAX_ERRORS * sizeof(struct al_err_events_field)));

	al_memset(&module_params, 0, sizeof(module_params));

	/* init module */
	al_sprintf(module_params.name, "PBS");
	...
	module_params.ic_regs_bases[0] = al_pbs_iofic_regs_get(handle->pbs_regs_base);
	module_params.ic_size = 1;
	module_params.fields_size = AL_ERR_EVENTS_PBS_MAX_ERRORS;
	module_params.fields = data->fields;
	module_params.fields_template = al_err_events_pbs_fields;
	...
	rev_id = al_pbs_dev_id_get(handle->pbs_regs_base);
...
int al_err_events_pbs_ints_enable(struct al_err_events_pbs_data *data)
{
	...
	al_iofic_config(module->ic_regs_bases[0],
			AL_INT_GROUP_A,
			INT_CONTROL_GRP_MASK_MSI_X);

	al_iofic_unmask(module->ic_regs_bases[0],
			AL_INT_GROUP_A,
			al_err_events_common_group_mask_get(module, 0, AL_INT_GROUP_A));
 ...
 ** Serdes 25G
 ******************************************************************************/
static int err_events_serdes_25g_collect(struct al_err_events_module *module,
				  ...
				  struct al_err_events_serdes_25g_init_params *params)
{
	...
	al_assert(data);
	al_assert(params);
	al_assert(sizeof(al_err_events_serdes_fields) == sizeof(data->fields));
	al_assert(sizeof(al_err_events_serdes_fields) ==
		 (AL_ERR_EVENTS_SERDES_MAX_ERRORS * sizeof(struct al_err_events_field)));

	rev_id = al_pbs_dev_id_get(handle->pbs_regs_base);
	al_assert(rev_id == PBS_UNIT_CHIP_ID_DEV_ID_ALPINE_V2);

	/* init module */
	al_memset(&module_params, 0, sizeof(module_params));
	al_sprintf(module_params.name, "SERDES-25G");
	...
	module_params.fields = data->fields;
	module_params.fields_template = al_err_events_serdes_fields;
	module_params.private_data = data;
 ...
 ** AVG Serdes
 ******************************************************************************/
static int err_events_serdes_avg_collect(struct al_err_events_module *module,
					 enum al_err_events_collect collect_mode)
{
	return al_err_events_common_module_iofic_collect(module, collect_mode, 0, AL_INT_GROUP_A);
}
				  ...
				  struct al_err_events_serdes_avg_init_params *params)
{
	...
	al_assert(data);
	al_assert(params);
	al_assert(params->complex_obj);
	al_assert(params->regs_base);
	al_assert(sizeof(al_err_events_serdes_fields) == sizeof(data->fields));
	al_assert(sizeof(al_err_events_serdes_fields) ==
		 (AL_ERR_EVENTS_SERDES_AVG_MAX_ERRORS * sizeof(struct al_err_events_field)));

	rev_id = al_pbs_dev_id_get(handle->pbs_regs_base);
	al_assert(rev_id == PBS_UNIT_CHIP_ID_DEV_ID_ALPINE_V3);
	...
	al_sprintf(module_params.name, "SERDES-AVG");
	...
	module_params.fields = data->fields;
	module_params.fields_template = al_err_events_serdes_avg_fields;
	module_params.private_data = data;
...
int al_err_events_serdes_avg_ints_enable(struct al_err_events_serdes_avg_data *data)
{
	...
	al_err_event_common_update_valid_fields(&data->module, 0, a, b, c, d);
...
static void err_events_ua_axi_mask(struct al_err_events_field *field)
{
	...
	struct al_err_events_unit_adapter_data *data =
			(struct al_err_events_unit_adapter_data *)module->private_data;
	...
	al_unit_adapter_int_cause_mask_set_ex(&data->unit_adapter, &data->int_fields);
}

static void err_events_ua_axi_error_collect(struct al_err_events_field *field,
					    uint32_t cause __attribute__((unused)))
{
	struct al_err_events_module *module = field->parent_module;
	struct al_err_events_unit_adapter_data *data =
			(struct al_err_events_unit_adapter_data *)module->private_data;
	struct al_unit_adapter_err_attr *axi_err = &data->axi_err;
...
static void err_events_ua_fields_collect(struct al_err_events_module *module,
					 enum al_err_events_collect collect_mode,
...
static int err_events_unit_adapter_collect(struct al_err_events_module *module,
					   enum al_err_events_collect collect_mode)
{
	uint32_t cause;
	struct al_err_events_unit_adapter_data *data =
	...
	al_unit_adapter_int_cause_get_and_clear_ex(&data->unit_adapter, &int_fields);

	if (int_fields.wr_err) {
		data->axi_err_type = AL_ERR_EVENTS_AXI_ERR_TYPE_WRITE;
		al_unit_adapter_axi_master_wr_err_attr_get_and_clear(&data->unit_adapter,
								     &data->axi_err);
	...
	if (int_fields.rd_err) {
		...
		al_unit_adapter_axi_master_rd_err_attr_get_and_clear(&data->unit_adapter,
								     &data->axi_err);
		err_events_ua_fields_collect(module, collect_mode, AL_INT_GROUP_A, 0);
	}

	if (int_fields.app_parity_error) {
		cause = al_unit_adapter_app_parity_status_get_and_clear(&data->unit_adapter);
...
static void err_events_ua_axi_err_print(struct al_unit_adapter_err_attr *axi_err,
	al_err_events_print_cb print_cb, unsigned int alpine_dev_id)
{
...
static void al_err_events_unit_adapter_print(struct al_err_events_module *module)
{
	al_err_events_print_cb print_cb = module->handle->print_cb;
	struct al_err_events_unit_adapter_data *data =
			(struct al_err_events_unit_adapter_data *)module->private_data;
...
static void al_err_events_unit_adapter_clear(struct al_err_events_module *module)
{
	struct al_err_events_unit_adapter_data *data =
			(struct al_err_events_unit_adapter_data *)module->private_data;
	...
	al_memset(&data->axi_err, 0, sizeof(data->axi_err));
				    ...
				    struct al_err_events_ua_init_params *params)
{
	...
	al_assert(data);
	al_assert(params);
	al_assert(sizeof(al_err_events_ua_fields) == sizeof(data->fields));
	al_assert(sizeof(al_err_events_ua_fields) ==
		 (AL_ERR_EVENTS_UNIT_ADAPTER_MAX_ERRORS * sizeof(struct al_err_events_field)));

	/* init data */
	al_unit_adapter_handle_init(&data->unit_adapter,
				    params->type,
				    params->ua_regs_base,
				    NULL, NULL, NULL, NULL);
	al_memset(&data->int_fields, 0, sizeof(struct al_unit_adapter_int_fields));
	...
	case AL_UNIT_ADAPTER_TYPE_ETH:
		al_sprintf(module_params.name, "ETH(%d)-Unit-Adapter", params->index);
		module_params.primary_module = AL_ERR_EVENTS_MODULE_ETH;
		break;
	case AL_UNIT_ADAPTER_TYPE_SSM:
		al_sprintf(module_params.name, "SSM(%d)-Unit-Adapter", params->index);
		module_params.primary_module = AL_ERR_EVENTS_MODULE_SSM;
		break;
	case AL_UNIT_ADAPTER_TYPE_SATA:
		al_sprintf(module_params.name, "SATA(%d)-Unit-Adapter", params->index);
	...
	module_params.fields = data->fields;
	module_params.fields_template = al_err_events_ua_fields;
	module_params.private_data = data;
	module_params.collect = err_events_unit_adapter_collect;
	module_params.print = al_err_events_unit_adapter_print;
	module_params.clear = al_err_events_unit_adapter_clear;
...
int al_err_events_ua_ints_enable(struct al_err_events_unit_adapter_data *data)
{
	...
	al_unit_adapter_int_enable(&data->unit_adapter, AL_TRUE);
