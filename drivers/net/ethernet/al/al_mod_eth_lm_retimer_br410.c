 ...
 ******************************************************************************/
static const uint32_t al_mod_eth_retimer_boost_addr[AL_ETH_RETIMER_CHANNEL_MAX] = {
...
static const struct al_mod_eth_lm_retimer_da_config
 ...
 ** Callbacks
 ******************************************************************************/
static int al_mod_eth_lm_retimer_br410_config(struct al_mod_eth_lm_retimer *handle,
	unsigned int channel,
	struct al_mod_eth_lm_retimer_config_params *params)
{
	int rc = 0;
	struct al_mod_eth_lm_retimer_da_config retimer_params;
	uint32_t boost_addr;
	uint8_t reg;
	uint8_t vod_val;

	al_mod_assert((((int)channel) >= 0) && (channel < 4));

	al_mod_memset(&retimer_params, 0, sizeof(retimer_params));

	boost_addr = al_mod_eth_retimer_boost_addr[channel];
	...
	if ((params->da_len != 1) && (params->da_len != 3)) {
		al_mod_warn("%s: Uncalibrated cable length (%dm) for retimer_type BR410 on this "
			"board is being used! Link might not be optimal.\n",
			__func__, params->da_len);
		al_mod_warn("Retimer params have been calibrated for 1m and 3m cables only\n");
	}
	...
	if (rc) {
		al_mod_err("%s: Error occurred (%d) while writing to eq_boost to retimer "
			"(bus-id %x i2c-addr %x)\n",
	...
	if (rc) {
		al_mod_err("%s: Error occurred (%d) while reading de_emphasis control reg from "
	...
	if (rc) {
		al_mod_err("%s: Error occurred (%d) while writing to de_emphasis control reg of "
			"retimer (bus-id 0x%x i2c-addr 0x%x internal_reg 0x%x)\n",
			__func__, rc, handle->i2c_bus, handle->i2c_addr,
			RETIMER_BR410_DE_EMPHASIS_CTRL_REG);
		return rc;
	...
	if (rc) {
		al_mod_err("%s: Error occurred (%d) while reading Vod control reg from retimer "
	...
	if (vod_val != retimer_params.vod) {
		...
		if (rc) {
			al_mod_err("%s: Error occurred (%d) while writing to Vod control reg in "
				"retimer (bus-id 0x%x i2c-addr 0x%x internal_reg 0x%x)\n",
	...
	const struct al_mod_eth_lm_retimer_params *params)
{
	...
	handle->config = al_mod_eth_lm_retimer_br410_config;
