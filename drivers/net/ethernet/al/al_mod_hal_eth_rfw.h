...
struct al_mod_eth_rfw_handle_init_params {
	/** CPU number uses this handle */
	unsigned int cpu;
	/** Ethernet Controller registers base address */
	void __iomem *ec_regs_base;
...
struct al_mod_eth_rfw_lrss_params {
...
struct al_mod_eth_rfw_rss_table_params {
	uint32_t index;
	uint32_t tid; /* 20 bit */
	uint8_t udma;
	uint8_t queue;
	al_mod_bool tid_cmd;
	al_mod_bool udma_cmd;
	al_mod_bool queue_cmd;
};
...
struct al_mod_eth_rfw_lrss_tables_params {
...
struct al_mod_eth_rfw_lrss_handle_init_params {
	const struct al_mod_eth_rfw_handle *fwd_handle;
	const struct al_mod_eth_rfw_lrss_tables_params *lrss_table_params;
};

struct al_mod_eth_rfw_lrss_info {
	uint32_t rss_ind_table_offset;
	uint32_t hash_output_bit_mask;
	uint8_t default_queue;
};

struct al_mod_eth_rfw_lrss_handle {
	const struct al_mod_eth_rfw_handle *fwd_handle;
	uint32_t num; /**< number of sub-tables to initialize */
	uint32_t table_size_order; /**< order of the size of each sub-table */
	uint8_t default_queue[AL_ETH_RFW_LRSS_TABLES_MAX]; /**< default queue index for
								    * each sub-table
								    */
	struct al_mod_eth_rfw_lrss_info lrss_info[AL_ETH_RFW_LRSS_TABLES_MAX];
};
...
uint32_t al_mod_eth_rfw_lrss_ind_table_offset_get(
	const struct al_mod_eth_rfw_lrss_handle *lrss_handle,
