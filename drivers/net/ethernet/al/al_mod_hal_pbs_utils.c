	...
	void __iomem *pbs_regs)
{
	...
	return (al_mod_reg_read32(&regs->unit.chip_id) & PBS_UNIT_CHIP_ID_DEV_ID_MASK) >>
	...
	void __iomem *pbs_regs)
{
	...
	return (al_mod_reg_read32(&regs->unit.chip_id) & PBS_UNIT_CHIP_ID_DEV_REV_ID_MASK) >>
	...
	unsigned int	num_cyc_spi)
{
	...
	if (dev_id == PBS_UNIT_CHIP_ID_DEV_ID_ALPINE_V1) {
		al_mod_reg_write32(&regs->unit.cfg_axi_conf_3, num_cyc);
		return;
	}

	al_mod_assert(num_cyc_spi_low == num_cyc_low);
	al_mod_assert((num_cyc_hi & (PBS_UNIT_CFG_AXI_CONF_3_TIMEOUT_HI_MASK >>
			PBS_UNIT_CFG_AXI_CONF_3_TIMEOUT_HI_SHIFT)) == num_cyc_hi);
	al_mod_assert((num_cyc_low & (PBS_UNIT_CFG_AXI_CONF_3_TIMEOUT_LOW_MASK >>
			PBS_UNIT_CFG_AXI_CONF_3_TIMEOUT_LOW_SHIFT)) == num_cyc_low);
	al_mod_assert((num_cyc_spi_hi & (PBS_UNIT_CFG_AXI_CONF_3_TIMEOUT_SPI_HI_MASK >>
			PBS_UNIT_CFG_AXI_CONF_3_TIMEOUT_SPI_HI_SHIFT)) == num_cyc_spi_hi);
	al_mod_assert(num_cyc_spi_med == 0);
