...
static const struct al_mod_err_events_field
	al_mod_err_events_eth_ec_fields[AL_ERR_EVENTS_ETH_EC_MAX_ERRORS] = {
	{
...
static const struct al_mod_err_events_field
	al_mod_err_events_eth_mac_fields[AL_ERR_EVENTS_ETH_MAC_MAX_ERRORS] = {
	{
...
static const struct al_mod_err_events_field
	al_mod_err_events_eth_v4_ec_fields[AL_ERR_EVENTS_ETH_V4_EC_MAX_ERRORS] = {
	{
		.name = "Multi Stream Start In Packet Error",
...
static const struct al_mod_err_events_field
	al_mod_err_events_eth_ec_sec_fields[AL_ERR_EVENTS_ETH_V4_EC_SEC_MAX_ERRORS] = {
	{
...
static const struct al_mod_err_events_field
	al_mod_err_events_eth_v4_mac_fields[AL_ERR_EVENTS_ETH_V4_MAC_MAX_ERRORS] = {
{
 ...
 ** EC
 ******************************************************************************/
static int err_events_eth_ec_collect(struct al_mod_err_events_module *module,
				     enum al_mod_err_events_collect collect)
{
	int ret;
	unsigned int iofic_group;
	unsigned int iofic_id;
	struct al_mod_err_events_eth_ec_data *data = module->private_data;
	struct al_mod_err_events_eth_mac_data *mac_data = &data->mac_data;
	struct al_mod_err_events_eth_ec_sec_data *sec_data = &data->sec_data;
	struct al_mod_err_events_eth_epe_data *epe_data = &data->epe_data;
	...
	if (collect != AL_ERR_EVENTS_COLLECT_POLLING) {
		/* Collect from MAC */
		ret = al_mod_err_events_module_collect(&mac_data->module, collect);
		if (ret)
			return ret;

		if (sec_data->module.enabled) {
			/* Collect from Secondary level EC */
			ret = al_mod_err_events_module_collect(&sec_data->module, collect);
			if (ret)
				return ret;
		}
		...
		if ((module->primary_index == 0) && (epe_data->module.enabled)) {
			/* Collect from EPE */
			ret = al_mod_err_events_module_collect(&epe_data->module, collect);
			if (ret)
				return ret;
		}
	}
	...
	for (iofic_id = 0; iofic_id < module->ic_size; iofic_id++)
		for (iofic_group = AL_INT_GROUP_A; iofic_group <= AL_INT_GROUP_D; iofic_group++) {
			ret = al_mod_err_events_common_module_iofic_collect(module, collect,
...
static int err_events_mac_init(struct al_mod_err_events_handle *handle,
			      struct al_mod_err_events_eth_ec_data *data,
			      struct al_mod_err_events_eth_ec_init_params *params);

static int al_mod_err_events_eth_ec_sec_init(struct al_mod_err_events_handle *handle,
			      struct al_mod_err_events_eth_ec_sec_data *data,
			      struct al_mod_err_events_eth_ec_init_params *params);

static void err_events_epe_init(struct al_mod_err_events_handle *handle,
			struct al_mod_err_events_eth_ec_data *data,
			struct al_mod_err_events_eth_ec_init_params *params)
{
	struct al_mod_err_events_eth_epe_init_params epe_params;
	struct al_mod_eth_epe_handle_init_params epe_handle_params;

	/* Initialize EPE module */
	al_mod_memset(&epe_params, 0, sizeof(struct al_mod_err_events_eth_epe_init_params));
	al_mod_memset(&epe_handle_params, 0, sizeof(struct al_mod_eth_epe_handle_init_params));

	epe_handle_params.eth_rev_id = al_mod_eth_rev_id_get(params->adapter);
	epe_handle_params.regs_base = al_mod_eth_epe_regs_get(params->adapter);
	epe_handle_params.type = EPE_TYPE_OUTER;

	epe_params.ec_intc_regs_base =
		al_mod_eth_iofic_regs_base_get(params->adapter, AL_ETH_IOFIC_LEVEL_PRIMARY);
	epe_params.handle_params = &epe_handle_params;
	epe_params.collect_mode = params->collect_mode;
	al_mod_err_events_eth_epe_init(handle, &data->epe_data, &epe_params);

	al_mod_info("Registered %s:%s errors\n", data->module.name, data->epe_data.module.name);
}
			      ...
			      struct al_mod_err_events_eth_ec_init_params *params)
{
	...
	rev_id = al_mod_eth_rev_id_get(params->adapter);

	if (rev_id < AL_ETH_REV_ID_4) {
		al_mod_assert(sizeof(al_mod_err_events_eth_ec_fields) <= sizeof(data->fields));
		al_mod_assert(sizeof(al_mod_err_events_eth_ec_fields) ==
			(AL_ERR_EVENTS_ETH_EC_MAX_ERRORS * sizeof(struct al_mod_err_events_field)));
	} else {
		al_mod_assert(sizeof(al_mod_err_events_eth_v4_ec_fields) <= sizeof(data->fields));
		al_mod_assert(sizeof(al_mod_err_events_eth_v4_ec_fields) ==
			(AL_ERR_EVENTS_ETH_V4_EC_MAX_ERRORS * sizeof(struct al_mod_err_events_field)));
	}
	al_mod_assert(params->adapter);
	...
	/* init module */
	al_mod_sprintf(module_params.name, "ETH(%d)-EC", params->eth_index);

	if (rev_id >= AL_ETH_REV_ID_4) {
		module_params.fields_size = AL_ERR_EVENTS_ETH_V4_EC_MAX_ERRORS;
		module_params.fields_template = al_mod_err_events_eth_v4_ec_fields;
	} else {
		module_params.fields_size = AL_ERR_EVENTS_ETH_EC_MAX_ERRORS;
		module_params.fields_template = al_mod_err_events_eth_ec_fields;
	}
	...
	al_mod_eth_ec_iofic_attrs_get(params->adapter, AL_ETH_IOFIC_LEVEL_PRIMARY,
	...
	al_mod_err_events_eth_ec_sec_init(handle, &data->sec_data, params);
...
static int err_events_eth_ec_sec_collect(struct al_mod_err_events_module *module,
				enum al_mod_err_events_collect collect)
{
	int ret;
	unsigned int iofic_group;
	...
	struct al_mod_err_events_eth_ec_sec_data *ec_sec_data =
		(struct al_mod_err_events_eth_ec_sec_data *)module->private_data;
	void __iomem *primary_ints_base =
		al_mod_eth_iofic_regs_base_get(ec_sec_data->adapter, AL_ETH_IOFIC_LEVEL_PRIMARY);

	if (collect == AL_ERR_EVENTS_COLLECT_POLLING)
		should_collect = AL_TRUE;
	else if (collect == AL_ERR_EVENTS_COLLECT_INTERRUPT)
		cause = al_mod_iofic_read_cause(primary_ints_base, AL_INT_GROUP_C);
	...
	if (cause_clr)
		al_mod_iofic_clear_cause(primary_ints_base,
			AL_INT_GROUP_C,
			cause_clr);
			      ...
			      struct al_mod_err_events_eth_ec_init_params *params)
{
	...
	al_mod_assert(data);
	al_mod_assert(params);
	al_mod_assert(sizeof(al_mod_err_events_eth_ec_sec_fields) == sizeof(data->fields));
	al_mod_assert(sizeof(al_mod_err_events_eth_ec_sec_fields) ==
		 (AL_ERR_EVENTS_ETH_V4_EC_SEC_MAX_ERRORS * sizeof(struct al_mod_err_events_field)));
	al_mod_assert(params->adapter);
	al_mod_assert(al_mod_eth_rev_id_get(params->adapter) >= AL_ETH_REV_ID_4);
	...
	/* init module */
	al_mod_memset(&module_params, 0, sizeof(module_params));
	al_mod_sprintf(module_params.name, "ETH(%d)-EC secondary", params->eth_index);
	module_params.primary_module = AL_ERR_EVENTS_MODULE_ETH;
	...
	module_params.collect_mode = params->collect_mode;
	module_params.ic_regs_bases[0] =
		al_mod_eth_iofic_regs_base_get(params->adapter, AL_ETH_IOFIC_LEVEL_SECONDARY);
	module_params.ic_size = 1;
	module_params.fields_size = AL_ERR_EVENTS_ETH_V4_EC_SEC_MAX_ERRORS;
	module_params.fields = data->fields;
	module_params.fields_template = al_mod_err_events_eth_ec_sec_fields;
	module_params.private_data = data;
	...
	al_mod_eth_ec_iofic_attrs_get(params->adapter, AL_ETH_IOFIC_LEVEL_SECONDARY,
				  AL_ETH_IOFIC_ATTRS_RUNTIME_CONFIG,
				  &a_mask,
				  &b_mask,
				  &c_mask,
 ...
 ** MAC
 ******************************************************************************/
static int err_events_eth_mac_collect(struct al_mod_err_events_module *module,
				      enum al_mod_err_events_collect collect)
{
	...
	unsigned int iofic_group;
	al_mod_bool should_collect = AL_FALSE;
	struct al_mod_err_events_eth_mac_data *data =
			(struct al_mod_err_events_eth_mac_data *)module->private_data;
	unsigned int iofic_id;

	if (collect == AL_ERR_EVENTS_COLLECT_POLLING) {
		should_collect = AL_TRUE;
	} else if (collect == AL_ERR_EVENTS_COLLECT_INTERRUPT) {
		cause = al_mod_iofic_read_cause(data->ec_iofic_regs_base, AL_INT_GROUP_A);
	...
	for (iofic_id = 0; iofic_id < module->ic_size; iofic_id++)
		for (iofic_group = AL_INT_GROUP_A; iofic_group <= AL_INT_GROUP_D; iofic_group++)
			al_mod_err_events_common_module_iofic_collect(
				module, collect, iofic_id, iofic_group);

	if (collect == AL_ERR_EVENTS_COLLECT_INTERRUPT)
		al_mod_iofic_clear_cause(data->ec_iofic_regs_base,
			     AL_INT_GROUP_A,
			     data->mac_int_bit);
...
/** MAC init params */
struct al_mod_err_events_eth_mac_init_params {
...
static int al_mod_err_events_eth_mac_init(struct al_mod_err_events_handle *handle,
			       struct al_mod_err_events_eth_mac_data *data,
			       struct al_mod_err_events_eth_mac_init_params *params)
{
	uint32_t a_mask, b_mask;
	uint32_t c_mask, d_mask;
	struct al_mod_err_events_module *module;
	...
	al_mod_assert(params->adapter);
	rev_id = al_mod_eth_rev_id_get(params->adapter);
	al_mod_assert(rev_id <= AL_ETH_REV_ID_4);

	if (rev_id >= AL_ETH_REV_ID_4) {
		al_mod_assert(sizeof(al_mod_err_events_eth_v4_mac_fields) <= sizeof(data->fields));
		al_mod_assert(sizeof(al_mod_err_events_eth_v4_mac_fields) ==
			(AL_ERR_EVENTS_ETH_V4_MAC_MAX_ERRORS * sizeof(struct al_mod_err_events_field)));
	} else {
		al_mod_assert(sizeof(al_mod_err_events_eth_mac_fields) <= sizeof(data->fields));
		al_mod_assert(sizeof(al_mod_err_events_eth_mac_fields) ==
			(AL_ERR_EVENTS_ETH_MAC_MAX_ERRORS * sizeof(struct al_mod_err_events_field)));
	}
	...
	data->ec_iofic_regs_base =
		al_mod_eth_iofic_regs_base_get(params->adapter, AL_ETH_IOFIC_LEVEL_PRIMARY);
	data->mac_obj = al_mod_eth_mac_obj_get(params->adapter);
	...
	if (rev_id >= AL_ETH_REV_ID_4) {
		...
		data->mac_idx = al_mod_eth_get_serdes_lane(params->adapter);
		al_mod_sprintf(module_params.name, "ETH v4(%d)-MAC(%d)",
			params->eth_index, data->mac_idx);
		module_params.fields_size = AL_ERR_EVENTS_ETH_V4_MAC_MAX_ERRORS;
		module_params.fields_template = al_mod_err_events_eth_v4_mac_fields;

		data->mac_int_bit = mac_ints[data->mac_idx];
	} else {
		data->mac_idx = 0;
		al_mod_sprintf(module_params.name, "ETH(%d)-MAC", params->eth_index);
		module_params.fields_size = AL_ERR_EVENTS_ETH_MAC_MAX_ERRORS;
		module_params.fields_template = al_mod_err_events_eth_mac_fields;

		data->mac_int_bit = AL_ETH_EC_IOFIC_GROUP_A_MAC_INT_IN;
	}

	al_mod_eth_mac_iofic_attrs_get(data->mac_obj,
				   AL_ETH_IOFIC_ATTRS_RUNTIME_CONFIG,
	...
	module_params.collect_mode = params->collect_mode;
	module_params.ic_regs_bases[0] = al_mod_eth_mac_iofic_regs_get(data->mac_obj);
	module_params.ic_size = 1;
	module_params.fields = data->fields;
	module_params.private_data = data;
...
static int err_events_mac_init(struct al_mod_err_events_handle *handle,
			      ...
			      struct al_mod_err_events_eth_ec_init_params *params)
{
	struct al_mod_err_events_eth_mac_init_params mac_params;

	al_mod_memset(&mac_params, 0, sizeof(struct al_mod_err_events_eth_mac_init_params));
	mac_params.adapter = params->adapter;
	mac_params.eth_index = params->eth_index;
	mac_params.collect_mode = params->collect_mode;

	al_mod_err_events_eth_mac_init(handle, &data->mac_data, &mac_params);
