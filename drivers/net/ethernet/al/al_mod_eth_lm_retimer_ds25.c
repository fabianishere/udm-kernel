...
static int al_mod_eth_lm_retimer_ds25_write_reg(struct al_mod_eth_lm_retimer	*handle,
					    uint8_t			reg_addr,
					    uint8_t			reg_mask,
					    uint8_t			reg_value)
{
...
static void al_mod_eth_lm_retimer_ds25_channel_deselect(struct al_mod_eth_lm_retimer	*handle)
{
...
static int al_mod_eth_lm_retimer_ds25_channel_select(struct al_mod_eth_lm_retimer	*handle,
						 uint8_t			channel)
{
	int rc = 0;
	...
	rc = al_mod_eth_lm_retimer_ds25_write_reg(handle,
...
config_done:
	if (rc)
		al_mod_eth_lm_retimer_ds25_channel_deselect(handle);

	return rc;
}

static int al_mod_eth_lm_retimer_ds25_channel_config(struct al_mod_eth_lm_retimer	*handle,
						 uint8_t			channel,
						 ...
						 uint8_t			config_size)
{
	...
	rc = al_mod_eth_lm_retimer_ds25_channel_select(handle, channel);
	if (rc)
...
config_done_no_unlock:
	if (rc)
		al_mod_err("%s: failed to access to the retimer\n", __func__);
...
static int al_mod_eth_lm_retimer_ds25_config(struct al_mod_eth_lm_retimer *handle,
	...
	struct al_mod_eth_lm_retimer_config_params *params)
{
	...
	rc = al_mod_eth_lm_retimer_ds25_channel_config(handle,
					channel,
					config_regs,
					config_regs_size);
...
static int al_mod_eth_lm_retimer_ds25_cdr_reset(struct al_mod_eth_lm_retimer *handle, unsigned int channel)
{
	int rc = 0;
...
static int al_mod_eth_lm_retimer_ds25_signal_detect(struct al_mod_eth_lm_retimer *handle,
						    unsigned int channel,
...
static int al_mod_eth_lm_retimer_ds25_cdr_lock(struct al_mod_eth_lm_retimer *handle,
					       unsigned int channel,
					       al_mod_bool *locked)
{
	int rc = 0;
...
static int al_mod_eth_lm_retimer_ds25_rx_eq_done(struct al_mod_eth_lm_retimer *handle,
					       unsigned int channel,
					       al_mod_bool *done)
{
	int rc = 0;
...
static int al_mod_eth_lm_retimer_ds25_speed_get(
					...
					enum al_mod_eth_lm_retimer_speed *speed)
{
	...
	*speed = AL_ETH_LM_RETIMER_SPEED_25G;
	...
	rc = al_mod_eth_lm_retimer_ds25_write_reg(
					handle,
	...
	const struct al_mod_eth_lm_retimer_params *params)
{
	...
	handle->config = al_mod_eth_lm_retimer_ds25_config;
	handle->reset = al_mod_eth_lm_retimer_ds25_cdr_reset;
	handle->signal_detect = al_mod_eth_lm_retimer_ds25_signal_detect;
	handle->cdr_lock = al_mod_eth_lm_retimer_ds25_cdr_lock;
	handle->rx_eq_done = al_mod_eth_lm_retimer_ds25_rx_eq_done;
	handle->speed_detect = al_mod_eth_lm_retimer_ds25_speed_get;
