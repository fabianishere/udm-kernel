...
#include "internal/al_mod_eth_lm_retimer_internal.h"
	...
	const struct al_mod_eth_lm_retimer_params *params)
{
	...
	al_mod_memset(handle, 0, sizeof(*handle));
	...
	case AL_ETH_LM_RETIMER_TYPE_BR_210:
		ret = al_mod_eth_lm_retimer_br210_handle_init(handle, params);
		break;
	case AL_ETH_LM_RETIMER_TYPE_BR_410:
		ret = al_mod_eth_lm_retimer_br410_handle_init(handle, params);
		break;
