...
static const uint32_t al_mod_eth_retimer_boost_lens[AL_ETH_RETIMER_DA_LENS_MAX] = {0, 1, 2, 3, 5};
...
static int al_mod_eth_lm_retimer_br210_config(struct al_mod_eth_lm_retimer *handle,
	...
	struct al_mod_eth_lm_retimer_config_params *params)
{
	...
	int rc = 0;
	uint8_t boost = 0;
	uint32_t boost_addr = al_mod_eth_retimer_boost_addr[channel];

	for (i = 0; i < AL_ETH_RETIMER_DA_LENS_MAX; i++) {
		if (params->da_len <= al_mod_eth_retimer_boost_lens[i]) {
	...
	rc = handle->i2c_write(handle->i2c_context,
				   ...
				   boost);
	if (rc) {
		al_mod_err("%s: Error occurred (%d) while writing retimer configuration "
			"(bus-id %x i2c-addr %x)\n",
	...
	const struct al_mod_eth_lm_retimer_params *params)
{
	...
	handle->config = al_mod_eth_lm_retimer_br210_config;
