				 ...
				 enum al_mod_err_events_collect collect)
{
	...
	al_mod_assert_msg(module->collect, "Undefined collect function for %s\n", module->name);
	al_mod_assert(collect != AL_ERR_EVENTS_COLLECT_DEFAULT);

	if (!module->enabled) {
		al_mod_err("%s: ERROR module %s is not enabled\n",
			__func__,
			module->name);
		return -EINVAL;
	}
...
{
	al_mod_assert(module);
	al_mod_assert_msg(module->print, "Undefined print function for %s\n", module->name);
...
{
	al_mod_assert(module);
	al_mod_assert_msg(module->clear, "Undefined clear function for %s\n", module->name);
			      ...
			      struct al_mod_err_events_handle_init_params *init)
{
	...
	al_mod_assert(init);
	al_mod_assert(init->pbs_regs_base);
	al_mod_assert(init->print_cb);
	al_mod_assert(init->error_cb);
	...
	handle->dev_id = al_mod_pbs_dev_id_get(handle->pbs_regs_base);
...
void al_mod_err_events_handle_print(struct al_mod_err_events_handle *handle)
{
	...
	for (module = handle->first_comp; module; module = module->next_comp)
		al_mod_err_events_module_print(module);
...
void al_mod_err_events_handle_clear(struct al_mod_err_events_handle *handle)
{
	...
	for (module = handle->first_comp; module; module = module->next_comp)
		al_mod_err_events_module_clear(module);
}
...
al_mod_bool al_mod_err_events_handle_test(struct al_mod_err_events_handle *handle)
{
	...
	al_mod_bool test_res;

	al_mod_assert(handle);

	for (module = handle->first_comp; module; module = module->next_comp) {
		test_res = al_mod_err_events_module_test(module);
		if (!test_res)
			ret = AL_FALSE;
	}
				 ...
				 enum al_mod_err_events_collect collect)
{
	...
	for (module = handle->first_comp; module; module = module->next_comp) {
		ret = al_mod_err_events_module_collect(module, collect);
		if (ret)
			return ret;
	}
...
void al_mod_err_events_handle_ints_mask(struct al_mod_err_events_handle *handle)
{
	...
	for (module = handle->first_comp; module; module = module->next_comp)
		al_mod_err_events_module_ints_mask(module);
}
