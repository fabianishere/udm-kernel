...
extern struct acl_hw_ops ar8337_acl_ops;
...
const struct ar8xxx_mib_desc ar8236_mibs[41] = {
		      ...
		      struct switch_port_link *link)
{
	...
	if(priv->chip->portno_to_phyno)
	{
		int phyno = priv->chip->portno_to_phyno(port);
		if(phyno >= 0 && !!(mdiobus_read(priv->mii_bus, phyno, MII_BMCR) & BMCR_PDOWN)) {
			link->power_down = 1;
			return;
		} else {
			link->power_down = 0;
...
ar8xxx_sw_set_pvid(struct switch_dev *dev, int port, int vlan)
{
	...
	if (priv->vt[priv->pvid[port]].untagged & (1 << port)) {
		priv->vt[priv->pvid[port]].untagged &= ~(1 << port);
		priv->vt[priv->pvid[port]].tagged |= (1 << port);
	}
		  ...
		  struct switch_val *val)
{
	...
	priv->vt[val->port_vlan].use_ivl = val->value.i;
	return 0;
}
...
{
	struct ar8xxx_priv *priv = swdev_to_ar8xxx(dev);
	val->value.i = priv->vt[val->port_vlan].use_ivl;
	return 0;
}
...
ar8xxx_sw_get_ports(struct switch_dev *dev, struct switch_val *val)
{
	...
	ports = priv->vt[val->port_vlan].vlan_entry;
	val->len = 0;
...
ar8xxx_sw_hw_apply(struct switch_dev *dev)
{
	...
	if (!priv->init) {
		...
		for (j = 0; j < AR8X16_MAX_VLANS; j++) {
			u8 vp = priv->vt[j].tagged | priv->vt[j].untagged;
...
ar8xxx_id_chip(struct ar8xxx_priv *priv)
{
	...
	case AR8XXX_VER_AR8327:
		...
		priv->hw_acl.ops = &ar8337_acl_ops;
		break;
...
/* COUNTER( _field, _sum_fields) */
#define UEXT_AR8XXX_MIB_SUMS(COUNTER) \
		COUNTER(rx_byte, {"RxBadByte", "RxGoodByte", NULL}) \
		COUNTER(tx_byte, {"TxByte", NULL}) \
		COUNTER(rx_total, {"RxUnicast", "RxMulti", "RxBroad", NULL}) \
		COUNTER(tx_total, {"TxUnicast", "TxMulti", "TxBroad", NULL}) \
		COUNTER(rx_bcast, {"RxBroad", NULL}) \
		COUNTER(tx_bcast, {"TxBroad", NULL}) \
		COUNTER(rx_mcast, {"RxMulti", NULL}) \
		COUNTER(tx_mcast, {"TxMulti", NULL}) \
		COUNTER(rx_pause, {"RxPause", NULL}) \
		COUNTER(tx_pause, {"TxPause", NULL}) \
		COUNTER(rx_jumbo, {"RxMaxByte", NULL}) \
		COUNTER(tx_jumbo, {"TxMaxByte", NULL}) \
		COUNTER(rx_dropped, {"RxOverFlow", "RxAlignErr", "RxRunt", "RxFragment", "RxTooLong", "Filtered", NULL}) \
		COUNTER(tx_dropped, {"TxUnderRun", "TxOverSize", "TxAbortCol", NULL}) \
		COUNTER(rx_error, {"RxOverFlow", "RxAlignErr", "RxRunt", "RxFragment", "RxTooLong", NULL}) \
		COUNTER(tx_error, {"TxUnderRun", "TxOverSize", "TxAbortCol", NULL})

/**
 * @note
 * Not every switch chip's mib counter matches uext mib counter, some of them have to be summed up first.
 */
#define COUNTER( _field, ...) static const char * mib_sum_##_field [] = __VA_ARGS__;
	UEXT_AR8XXX_MIB_SUMS(COUNTER)
#undef COUNTER

/**
 * @brief Get MIB counter by names
 */
static inline uint64_t ar8xxx_sw_get_counter_sum_by_name(struct ar8xxx_priv *priv, int port,
							 const char **mib_names)
{
	const struct ar8xxx_chip *chip = priv->chip;
	uint64_t result = 0;
	uint64_t *mib_stats = &priv->mib_stats[port * chip->num_mibs];
	int i = 0;

	while (NULL != *mib_names) {
		for (i = 0; i < chip->num_mibs; ++i) {
			if (!strcmp(chip->mib_decs[i].name, *mib_names)) {
				result += mib_stats[i];
				goto next_name;
			}
		}
		pr_warn("ar8xxx: no counter with name %s found\n", *mib_names);
	next_name:
		++mib_names;
	}
	return result;
 ...
 *
 */
int ar8xxx_uext_get_port_mib(struct switch_dev *dev, struct switch_port_mib *mib,
				  int port)
{
	struct ar8xxx_priv *priv = swdev_to_ar8xxx(dev);
	const struct ar8xxx_chip *chip = priv->chip;
...
#define COUNTER(_field, ...) \
	mib->_field = ar8xxx_sw_get_counter_sum_by_name(priv, port, mib_sum_##_field);
	UEXT_AR8XXX_MIB_SUMS(COUNTER)
#undef COUNTER
...
ar8xxx_phy_read_status(struct phy_device *phydev)
{
	...
	if (chip_is_ar8337(priv) || chip_is_ar8327(priv)) {
		port = -EINVAL;
		if (priv->chip->phyaddr_to_portno) {
			/* phydev->addr != port number, there has to be translation table for it */
			port = priv->chip->phyaddr_to_portno(phydev->mdio.addr);
		}
...
ar8xxx_phy_config_aneg(struct phy_device *phydev)
{
	...
	if (phydev->mdio.addr == 0 || chip_is_ar8337(priv) || chip_is_ar8327(priv)) {
		return 0;
	}
