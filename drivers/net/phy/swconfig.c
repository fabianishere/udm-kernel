			...
			struct switch_val *val)
{
	...
	int rc = swconfig_uext_reset(dev);
	if (rc) {
		return rc;
	}
...
enum global_defaults {
	GLOBAL_APPLY,
	GLOBAL_RESET,
#define UEXT_ATT(_name, _desc, _enum, _type, _get, _set) GLOBAL_##_enum,
	UBNT_EXT_GLOBAL_ATT(UEXT_ATT)
#undef UEXT_ATT
};
...
enum port_defaults {
	PORT_PVID,
	PORT_LINK,
#define UEXT_ATT(_name, _desc, _enum, _type, _get, _set) PORT_##_enum,
	UBNT_EXT_PORT_ATT(UEXT_ATT)
#undef UEXT_ATT
};

#ifdef CONFIG_SWCONFIG_UBNT_EXT
enum svlan_defaults {
#define UEXT_ATT(_name, _desc, _enum, _type, _get, _set) SVLAN_##_enum,
	UBNT_EXT_SVLAN_ATT(UEXT_ATT)
#undef UEXT_ATT
};
#endif /* CONFIG_SWCONFIG_UBNT_EXT */
...
#define UEXT_ATT(_name, _desc, _enum, _type, _get, _set) \
	,[GLOBAL_##_enum] = { \
		.type = _type, \
		.name = _name, \
		.description = _desc, \
...
#ifdef CONFIG_SWCONFIG_UBNT_EXT
static struct switch_attr default_svlan[] = {
	#define UEXT_ATT(_name, _desc, _enum, _type, _get, _set) \
	[SVLAN_##_enum] = { \
		.type = _type, \
		.name = _name, \
...
#define UEXT_ATT(_name, _desc, _enum, _type, _get, _set) \
	set_bit(PORT_##_enum, &dev->def_port);
	UBNT_EXT_PORT_ATT(UEXT_ATT)
#undef UEXT_ATT

#define UEXT_ATT(_name, _desc, _enum, _type, _get, _set) \
	set_bit(SVLAN_##_enum, &dev->def_svlan);
	UBNT_EXT_SVLAN_ATT(UEXT_ATT)
#undef UEXT_ATT
...
#define UEXT_ATT(_name, _desc, _enum, _type, _get, _set) \
	set_bit(GLOBAL_##_enum, &dev->def_global);
	UBNT_EXT_GLOBAL_ATT(UEXT_ATT)
#undef UEXT_ATT
...
#ifdef CONFIG_SWCONFIG_UBNT_EXT
	[SWITCH_ATTR_OP_SVLAN] = { .type = NLA_U32 },
#endif /* CONFIG_SWCONFIG_UBNT_EXT */
...
swconfig_list_attrs(struct sk_buff *skb, struct genl_info *info)
{
	...
	case SWITCH_CMD_LIST_VLAN:
		...
		break;
#ifdef CONFIG_SWCONFIG_UBNT_EXT
	case SWITCH_CMD_LIST_SVLAN:
		alist = &dev->ops->attr_svlan;
		def_list = default_svlan;
		def_active = &dev->def_svlan;
		n_def = ARRAY_SIZE(default_svlan);
		break;
#endif
		...
		struct switch_val *val)
{
	...
	case SWITCH_CMD_GET_VLAN:
		...
		break;
#ifdef CONFIG_SWCONFIG_UBNT_EXT
	case SWITCH_CMD_SET_SVLAN:
	case SWITCH_CMD_GET_SVLAN:
		alist = &dev->ops->attr_svlan;
		def_list = default_svlan;
		def_active = &dev->def_svlan;
		n_def = ARRAY_SIZE(default_svlan);
		if (!info->attrs[SWITCH_ATTR_OP_SVLAN])
			goto done;
		val->port_vlan = nla_get_u32(info->attrs[SWITCH_ATTR_OP_SVLAN]);
		if (val->port_vlan >= dev->vlans)
			goto done;
		break;
#endif
...
#ifdef CONFIG_SWCONFIG_UBNT_EXT
	link->power_down = !!tb[SWITCH_LINK_FLAG_POWER_DOWN];
#endif /* CONFIG_SWCONFIG_UBNT_EXT */
...
#ifdef CONFIG_SWCONFIG_UBNT_EXT
	if (link->power_down) {
		if (nla_put_flag(msg, SWITCH_LINK_FLAG_POWER_DOWN))
			goto nla_put_failure;
	}
#endif /* CONFIG_SWCONFIG_UBNT_EXT */
	if (link->link) {
...
swconfig_get_attr(struct sk_buff *skb, struct genl_info *info)
{
...
#ifdef CONFIG_SWCONFIG_UBNT_EXT
	if (attr->type == SWITCH_TYPE_MIB) {
		val.value.mib = &dev->mibbuf;
		memset(&dev->mibbuf, 0, sizeof(dev->mibbuf));
	}
#endif
	...
	case SWITCH_TYPE_LINK:
		...
		break;
#ifdef CONFIG_SWCONFIG_UBNT_EXT
	case SWITCH_TYPE_MIB:
		err = swconfig_uext_send_mib(msg, info,
				SWITCH_ATTR_OP_VALUE_MIB, val.value.mib);
		if (err < 0)
			goto nla_put_failure;
		break;
#endif
...
#ifdef CONFIG_SWCONFIG_UBNT_EXT
	{
		.cmd = SWITCH_CMD_GET_SVLAN,
		.doit = swconfig_get_attr,
		.policy = switch_policy,
	},
	{
		.cmd = SWITCH_CMD_SET_SVLAN,
		.doit = swconfig_set_attr,
		.policy = switch_policy,
	},
	{
		.cmd = SWITCH_CMD_LIST_SVLAN,
		.doit = swconfig_list_attrs,
		.policy = switch_policy,
	},
#endif
...
#ifdef CONFIG_SWCONFIG_UBNT_EXT
	err = swconfig_uext_register(dev);
	if (err) {
		return err;
	}
#endif
