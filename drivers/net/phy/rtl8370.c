...
{
	int idx = (RTL8370_PHY_ADDR_CPU_0 == phydev->addr) ? 0 : 1;
...
static int rtl8370_phy_probe(struct phy_device *phydev)
{
	...
	if (RTL8370_PHY_ADDR_CPU_0 != phydev->addr && RTL8370_PHY_ADDR_CPU_1 != phydev->addr) {
		return -ENODEV;
	}
...
found:
	priv->use_count++;
	idx = (RTL8370_PHY_ADDR_CPU_0 == phydev->addr) ? 0 : 1;
...
/**
 * @brief PHY driver's init
 *
 * @return int - error from errno.h, 0 on success
 */
int __init rtl8370_init(void)
{
	return phy_driver_register(&rtl870_phy_driver);
}

/**
 * @brief PHY driver's exit
 */
void __exit rtl8370_exit(void)
{
	phy_driver_unregister(&rtl870_phy_driver);
}

module_init(rtl8370_init);
module_exit(rtl8370_exit);
