...
#define AR8236_STATS_TXUNICAST		0xa8
#define AR8236_STATS_RXUNICAST		0xac
...
#define AR8XXX_NUM_ACL_ENTRIES	96
...
struct ar8xxx_chip {
	...
	int (*acl_flush)(struct ar8xxx_priv *priv);
...
/**
 * HW representation of ACL record
 */
#define ACL_RULE_PART_ACT_SIZE 3
#define ACL_RULE_PART_VLU_SIZE 5
#define ACL_RULE_PART_MSK_SIZE 5

typedef union {
	struct {
		uint32_t vlu[ACL_RULE_PART_VLU_SIZE];
		uint32_t msk[ACL_RULE_PART_MSK_SIZE];
		uint32_t act[ACL_RULE_PART_ACT_SIZE];
	} part;
	uint32_t data[13];
} ar8327_acl_entry_hw_t;

typedef enum {
	ACL_RULE_PART_VLU = 0x00,
	ACL_RULE_PART_MSK = 0x01,
	ACL_RULE_PART_ACT = 0x02,
	ACL_RULE_PART_CNT
...
int
ar8xxx_uext_get_port_mib(struct switch_dev *dev,
			struct switch_port_mib *mib,
			int port);
